...
$("#add-exercise").on('click', function(){
  ...
  storageBreakdown().exercise.forEach(function(exercise, index){
    populateTableFromStorage(index, exercise, "diary-exerercise")
  })
});
...
function storageUpdater(object, table){
  ...
  storage[table].push(object)
}

function populateTableFromStorage(index, object, table) {
  var row = "<tr id='" + index +"'></td><td contenteditable='true' class='enclosed-cells food-cell'>" + object.name + "</td><td contenteditable='true' class='enclosed-cells calorie-cell'>" + object.calories + "</td><td><button class='delete-button'><span class=' glyphicon glyphicon-remove'></span></button></td></tr>"
  $(table).prepend(row)
