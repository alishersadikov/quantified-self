...
function displayDiaryTables() {
  ...
  $('#breakfast-tbody').slice(0).empty()
  storageBreakdown().breakfast.forEach(function(breakfast, index){
    var row = "<tr id='" + index +"'></td><td contenteditable='true' class='enclosed-cells food-cell'>" + breakfast.name + "</td><td contenteditable='true' class='enclosed-cells calorie-cell'>" + breakfast.calories + "</td><td><button class='delete-button'><span class='glyphicon glyphicon-remove'></span></button></td></tr>";
    $('#breakfast-tbody').prepend(row);
  });

  $('#lunch-tbody').slice(0).empty()
  storageBreakdown().lunch.forEach(function(lunch, index){
    var row = "<tr id='" + index +"'></td><td contenteditable='true' class='enclosed-cells food-cell'>" + lunch.name + "</td><td contenteditable='true' class='enclosed-cells calorie-cell'>" + lunch.calories + "</td><td><button class='delete-button'><span class='glyphicon glyphicon-remove'></span></button></td></tr>";
    $('#lunch-tbody').prepend(row);
  });

  $('#dinner-tbody').slice(0).empty()
  storageBreakdown().dinner.forEach(function(food, index){
    var row = "<tr id='" + index +"'></td><td contenteditable='true' class='enclosed-cells food-cell'>" + food.name + "</td><td contenteditable='true' class='enclosed-cells calorie-cell'>" + food.calories + "</td><td><button class='delete-button'><span class='glyphicon glyphicon-remove'></span></button></td></tr>";
    $('#dinner-tbody').prepend(row);
  });

  $('#snacks-tbody').slice(0).empty()
  storageBreakdown().snacks.forEach(function(food, index){
    var row = "<tr id='" + index +"'></td><td contenteditable='true' class='enclosed-cells food-cell'>" + food.name + "</td><td contenteditable='true' class='enclosed-cells calorie-cell'>" + food.calories + "</td><td><button class='delete-button'><span class='glyphicon glyphicon-remove'></span></button></td></tr>";
    $('#snacks-tbody').prepend(row);
  });
}
